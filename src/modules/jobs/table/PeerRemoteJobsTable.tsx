...
const createPeerRemoteJobsColumns = (
  t: (key: string) => string,
): ColumnDef<Job>[] => [
  {
    accessorKey: "Type",
  ...
  },
  {
    accessorKey: "Status",
    header: ({ column }) => (
      <DataTableHeader column={column}>{t("common.status")}</DataTableHeader>
    ),
...
export default function PeerRemoteJobsTable({
  ...
  const { t } = useI18n();
  const PeerRemoteJobsColumns = createPeerRemoteJobsColumns(t);
