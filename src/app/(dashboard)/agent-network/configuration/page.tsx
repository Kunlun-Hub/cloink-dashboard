      ...
      <VerticalTabs value={tab} onChange={onTabChange}>
        <VerticalTabs.List>
          <VerticalTabs.Trigger value={TAB_BUDGET_SETTINGS}>
            <Gauge size={14} />{t("agentNetwork.globalLimits")}</VerticalTabs.Trigger>
          <VerticalTabs.Trigger value={TAB_LOG_SETTINGS}>
            <ScrollText size={14} />{t("agentNetwork.logCollection")}</VerticalTabs.Trigger>
          {canReadClusters && (
            <VerticalTabs.Trigger value={TAB_CLUSTERS}>
              <ServerIcon size={14} />{t("nav.clusters")}</VerticalTabs.Trigger>
          )}
        </VerticalTabs.List>
        <RestrictedAccess
          page={"Configuration"}
